...
VALUES
  ...
  -- Q1 Easy: answer = "primary key" — question never says "primary key"
  ...
  -- Q2 Easy: answer = "SQL" — question never says "SQL"
  ...
  -- Q3 Easy: answer = "foreign key" — question never says "foreign key"
  ...
  -- Q4 Easy: answer = "normalization" — question never says "normalization"
  ('What is the name of the design process that organises a relational schema into smaller, well-structured tables to eliminate redundant data and reduce update anomalies?',
   ...
   'normalization',
   'easy', 4,
   'Think of it as the process of putting data into its most organised, non-repetitive form — named after the concept of "normal forms".',
   '["normalisation", "data normalisation", "db normalization", "normal form process"]'),

  -- Q5 Easy: answer = "index" — question never says "index"
  ...
  -- Q6 Easy: answer = "NULL" — question never says "NULL" or "null"
  ...
  -- Q7 Easy: answer = "RDBMS" — question never says "RDBMS" or "relational database"
...
-- ─── MEDIUM STAGE (7 questions) ───────────────────────────────────────────────

  -- Q8 Medium: answer = "transaction" — question never says "transaction"
  ...
  -- Q9 Medium: answer = "ACID" — question never says "ACID"
  ('What four-letter acronym names the set of properties — Atomicity, Consistency, Isolation, and Durability — that guarantee reliable processing of database operations?',
   'ACID stands for Atomicity, Consistency, Isolation, and Durability — the four key properties that guarantee reliable and predictable database transactions.',
   'ACID',
   'medium', 2,
  ...
  -- Q10 Medium: answer = "JOIN" — question never says "join" or "JOIN"
  ...
  -- Q11 Medium: answer = "denormalization" — question never says "denormalization"
  ('What is the term for the deliberate process of introducing controlled redundancy into a database schema — for example by merging tables — in order to improve read query performance?',
  ...
  -- Q12 Medium: answer = "stored procedure" — question never says "stored procedure"
  ...
  -- Q13 Medium: answer = "view" — question never says "view"
  ...
  -- Q14 Medium: answer = "TRUNCATE" — question never says "TRUNCATE" or "truncate"
...
-- ─── HARD STAGE (7 questions) ─────────────────────────────────────────────────

  -- Q15 Hard: answer = "deadlock" — question never says "deadlock"
  ...
  -- Q16 Hard: answer = "CAP theorem" — question never says "CAP" or "theorem"
  ...
  -- Q17 Hard: answer = "B-tree" — question never says "B-tree" or "balanced tree"
  ...
  -- Q18 Hard: answer = "MVCC" — question never says "MVCC" or "multi-version"
  ('What concurrency control technique used by databases such as PostgreSQL and Oracle allows readers and writers to operate simultaneously without blocking each other by maintaining multiple timestamped snapshots of each row?',
   ...
   'MVCC',
   'hard', 4,
   'The acronym stands for a method where the database keeps several copies of a row — one per version — so readers never block writers.',
   '["multi version concurrency control", "multiversion concurrency", "multi-version concurrency control", "multi version control"]'),

  -- Q19 Hard: answer = "sharding" — question never says "sharding" or "shard"
  ...
  -- Q20 Hard: answer = "OLAP" — question never says "OLAP" or "analytical"
  ('What category of database workload is optimised for executing complex, multi-dimensional queries over large historical datasets to support business intelligence and reporting, as opposed to short, frequent transactional updates?',
   ...
   'OLAP',
   'hard', 6,
   'Think of it as the type of processing used in data warehouses and dashboards — the opposite of everyday transaction processing.',
   '["online analytical processing", "olap processing", "analytical processing", "data warehouse processing"]'),

  -- Q21 Hard: answer = "execution plan" — question never says "execution plan" or "plan"
  ('What is the name for the step-by-step strategy that a database engine generates internally — showing which indexes to use, how to order table scans, and how to perform joins — before it actually runs a SQL query?',
   ...
   'execution plan',
   'hard', 7,
   'You can reveal it by prepending the word EXPLAIN to any SQL query — it shows the engine''s internal road map.',
   '["query plan", "explain plan", "query execution plan", "execution strategy", "query plan"]');


-- ─── BADGES (progression emojis: seed → sprout → book → target → bolt → fire → gem → brain → medal → star → trophy → crown) ───
